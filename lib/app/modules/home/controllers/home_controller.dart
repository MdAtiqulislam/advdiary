...
var notice="".obs;
  ...
  Future<void> onInit() async {
    ...
    super.onInit();
    await getUserData();
    await getPackageInfo().then((value) async { await getHomeData().then((v) async {
      await getNotice();
    });});
  ...
  void newSubscribe({required String amount})async {
    ...
    try {
      var responses=await RemoteServices.getRequest(endPoint: endPoint,parameters: parameters);
      if(responses!=null){
        Get.put(BkashPaymentController()).url.value=responses["bkash_url"];
        await Get.toNamed(Routes.BKASH_PAYMENT)?.then((value) async {
          await getUserData();
          await getPackageInfo().then((value) async { await getHomeData();});
        });
      }else{
        CustomSnackBar(
            isSuccess: false,
  ...
  Future<void> getUpComingCaseDate() async {
    ...
    try {
      ...
      if(response!=null){
        ...
        if((upcomingCaseDateList.value.data??[]).isNotEmpty)upcomingCase.value=upcomingCaseDateList.value.data?.first??SingleNextCaseTimelineModel();
