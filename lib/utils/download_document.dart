...
Future<Directory?> getDownloadDirectory() async {
  if (Platform.isAndroid) {
    ...
    final customFolderPath = '$downloadsPath/Advocates Diary';
...
bool hasExtension(String fileName) {
  ...
  return parts.length > 1 && parts.last.isNotEmpty;
}
...
Future<void> downloadFile({
  ...
  try {
    ...
    // Get app-specific download folder
    final downloadsDir = await getAppDownloadDirectory();
    if (downloadsDir == null) throw Exception("Cannot access download folder");
...
Future<Directory?> getAppDownloadDirectory() async {
  ...
  if (baseDir != null) {
    final customFolder = Directory('${baseDir.path}/Advocates Diary');
    if (!await customFolder.exists()) {
      await customFolder.create(recursive: true);
    }
    return customFolder;
